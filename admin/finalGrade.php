<?php
    ...
    WHERE `Id`='$_SESSION[csId]'");
	$rowi = mysqli_fetch_array($query);
    $query2 = mysqli_query($con,"SELECT * FROM `tblgrade` WHERE `classId`='".$rowi['classId']."' AND `matricNo`='".$rowi['matricNo']."'");
    ...
    if(isset($_POST['submit'])){
        ...
        $subjects=mysqli_query($con,"SELECT `tblclass_sub`.`subjectCode`AS`subCode`,
        `tblclass_sub`.`classId`,`tblcourse`.`courseTitle`,`tblcourse`.`courseCode`,`tblcourse`.`courseUnit`
                                    FROM `tblclass_sub`
                                    ...
                                    WHERE `tblclass_sub`.`classId`=".$rowi['classId']."
												...
												while ($row=mysqli_fetch_array($subjects)) {
                                                    ...
                                                    $prelimgrade =floatval($_POST['sg'.$row['courseCode']]);
                                                if ($prelimgrade > 0) {
                                                    $query = mysqli_query($con, "UPDATE `tblgrade` SET `finalGrade`='".$prelimgrade."' WHERE `classId`='".$row['classId']."' AND `subjectCode`='".$row['subCode']."'");
                                                }                              
        }//while
        if($query === false){
            ...
            $statusMsg= 'Adding Final Grade is Successful!';
            header("location:viewGrades.php?statusMsg=".$statusMsg);
            exit(); 
