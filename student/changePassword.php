<?php
	...
	if(isset($_POST['submit'])){
		...
		$query=mysqli_query($con,"select * from `tblstudent` where matricNo='$staffId' and password='$cpassword'");
		$row=mysqli_fetch_array($query);
		if($row > 0){
		$ret=mysqli_query($con,"update `tblstudent` set password='$newpassword' where `matricNo`='$staffId'");
